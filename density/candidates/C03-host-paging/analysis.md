...
Per idle sandbox at the **500 ms tier**: private DRAM from the warm figure (`m_fixed + m_kproc + F_written + P_proc`, 167 MiB in the model) to `m_fixed` plus the hot set kept resident. The hot set is a parameter: REAP measured 8–99 MB (24 MB average) working sets for restored serverless functions, so the model keeps **24 MiB** resident, giving an idle figure of about **26 MiB**, *estimated*; the first draft's 3 MiB had no provenance. At the **5 s tier**, a fully evicted kernelet holds `m_fixed` only, about 2 MiB, and a sandbox with no written state can be destroyed and re-created from its image instead. At the **50 ms tier** NVMe is too slow for a working-set read (30 ms of a 60 MiB read at 2 GB/s before any fault), so eviction goes to the compressed pool (C07) and the miss cost, 100 µs per random NVMe read against 1–2 µs per decompression, is the argument.
...
Recorded here, not applied: C13's kernelet-physical addresses; C02's reclaim subsystem and grain return; a swap device model in the endovisor with a per-sandbox backing file or compressed pool; an idle signal on the control half (`Kernelet::idle_for()`) that accounts for `timer_arm` deadlines; the wake record; `KerneletStats` gains evicted and resident counts; the rule that pinned runs are not evictable.
