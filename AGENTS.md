...
- `src/blueprint/linux-mode/` ("Kernelets in Linux") is a standalone chapter that mirrors the Design chapter's page order for a Linux host, defines its own terms (carrier, root carrier, gate, kernelet stack, model and cache, seat, eviction), and gives its Linux source references as links to elixir.bootlin.com pinned at v6.12.
