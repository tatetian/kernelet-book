...
- `src/blueprint/linux-mode/` ("Kernelets in Linux") is a standalone chapter that mirrors the Design chapter's page order for a Linux host, defines its own terms (carrier, root carrier, virtual CPU, gate, kernelet stack, model and cache, upcall, the mirror, eviction), and gives its Linux source references as links to elixir.bootlin.com pinned at v6.12.
