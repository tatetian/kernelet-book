...
Asterinas Kernelets are a third point in that space, and this book calls the idea **API virtualization**: rather than virtualizing the hardware beneath a kernel, or multiplexing one kernel above its syscall table, it virtualizes the interface a kernel is written against, so that each tenant gets a kernel of its own and the machine underneath stays real. The Asterinas kernel is a *framekernel*: all `unsafe` code lives in a small framework, OSTD, and the roughly 180,000 lines of kernel above it are safe Rust written against OSTD's API. That API is the thing to virtualize. A **kernelet** is the unmodified kernel compiled against an OSTD-shaped API that is provided not by OSTD but by the **endovisor**, a component inside the host kernel. The endovisor re-exports what is harmless, re-implements the items that touch global state, and omits what a kernelet must never have, so that reaching for it is a build error. Where a hypervisor sits *beneath* its guests in hardware, the endovisor sits *inside* the host kernel and *beside* the kernelets, at the same privilege level, and its boundary is the language: the crate graph decides what a kernelet can name, and the type system decides what may cross. The price is an OSTD-shaped API to keep in step with OSTD, and a boundary that rests on the compiler and on OSTD's soundness rather than on hardware, with per-kernelet page tables as the backstop.
