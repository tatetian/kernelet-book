...
That reversal is the point. `munmap` and `mprotect` are calls the kernelet services itself, so the model is authoritative and Linux's tables are invalidated by the same code that changed it. The stale-hit hazard does not arise for them, because there is no second structure to go stale.

One path escapes that, and it is the one the hazard came from. A tenant killed by a signal, or by the host's out-of-memory killer, or by the oops of a neighboring bug, has its address space torn down with no service call anywhere, and the model outlives the mapping. Linux does call an area's owner on removal, so the hole is closable, but it has to be closed deliberately rather than by the argument above. The structural miss does not arise either: an address the kernelet never supplied is simply not in the model, and the model's own miss path is the kernel proper's fault handler.
...
Two things about that precedent do not carry, and "one store per slice" hides both. Linux's per-process mapping uses an index **reserved in the architecture's documented address-space layout**; an out-of-tree design has no reserved index and would be squatting on a documented hole that nothing stops a later kernel from allocating — which is the same objection the [loading page](loading.md#own-entries) raises against writing page-table entries directly, and it applies here too. And a new address space has its kernel half seeded from the host's own, so the alias is **not** inherited across `fork` or `exec` and must be re-installed for each one, by code the design does not yet have.
