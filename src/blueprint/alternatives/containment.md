...
**What it costs.** The no-patch route prints a full oops per fault before the notifier runs, and the console level is raised and never restored, so a tenant that faults in a loop can drown the machine's console. That is the real argument for the patch, and it is an argument about fairness rather than about function.
