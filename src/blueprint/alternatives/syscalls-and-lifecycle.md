...
**What it costs.** A register-frame copy per crossing, which is what returning from a signal already costs. And four places where it can die: the floating-point state, whose exported swap helper is present only on a kernel built with virtualization support; Linux's validation on the way out of a register frame it did not write; a signal or a forced trap arriving on a carrier that is *between* tenant threads; and the concentration of a tenant's concurrency onto a few carriers, which makes the model page table's exclusive cursor lock worse rather than better.
...
## The address space is the sandbox {#anchor-on-mm}

*Attacks:* the fact that interception is a property of a task, which Linux creates, copies, replaces and destroys at four moments a module must each get right. *Helps:* Linux only.

Anchor the kernelet on the **address space** instead. Threads then share a kernelet for free, because sharing an address space is what a thread is. The lifetime rules become the address space's, which Linux already runs. And the security argument changes shape in a way worth stating: losing your kernelet means losing your memory, rather than escaping your kernelet.

**Verdict.** Dead as a design, and its ideas are worth harvesting. Most of the defects it repairs were repaired by the corrections this exploration made along the way: the inheritance it fixes turned out to be correct already, and the entry points it enumerates are one point. What survives is the framing, and the cgroup it uses for accounting, both of which belong in the conservative design.
