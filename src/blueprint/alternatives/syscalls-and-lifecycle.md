...
One flag has to come with it, and it only works in one of the two designs. Linux copies a raw-frame-number area's page tables into a forked child, so a tenant that forks shares its frames with its child, writably, with no copy-on-write. That is a data leak in the design as specified. The flag that stops the copy gives the child an **empty** area rather than a copy of the parent's, which is not what `fork` means — so it is a fix only because the kernelet repopulates the child from its own page table on the first fault. Under the previous chapter's decision as it stands, with an alias and an index and no model, the same flag would give a child zeroes where its parent had data.
