...
1. **Check the host.** Refuse if `/proc/sys/kernel/panic_on_oops` is set ([why](faults-and-reclamation.md#fault)), if `fs.suid_dumpable` is not 0 ([why](endovisor.md#abi)), if `debug.exception-trace` is on ([why](virtualizing-ostd/user-mode.md#exceptions)), or if the endovisor's device is missing.
...
4. **Open `/dev/kernelet`** and `KERNELET_CREATE` with the kind, the number of virtual CPUs, the memory ceiling and the command line. Ask for the sandbox's **exec descriptor** with `KERNELET_EXEC_FD`.
