...
4. **Open `/dev/kernelet`** and `KERNELET_CREATE` with the kind, the number of virtual CPUs, the memory ceiling, the maximum number of tasks (from the bundle, or by default enough kernelet stacks to fill a quarter of `memory.max`) and the command line. Ask for the sandbox's **exec descriptor** with `KERNELET_EXEC_FD`.
