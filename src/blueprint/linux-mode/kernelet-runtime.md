...
6. **Fork the process that will become the root carrier.** In the child: move into the control group; enter the namespaces and the user the sandbox should run as, a user identity dedicated to this sandbox, so that no other process on the host may signal or inspect its carriers; clear the *dumpable* attribute, which closes `ptrace` and `/proc` access to same-user processes as well; leave the address-space limit unlimited; set *no new privileges* and install the seccomp filter that closes the [legacy vsyscall page](virtualizing-ostd/user-mode.md); and execute the exec descriptor with `execveat()`. The endovisor's program loader takes over from there, and the child never returns to user space.
