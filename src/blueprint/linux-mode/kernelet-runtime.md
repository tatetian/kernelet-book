...
1. **Check the host.** Refuse if `/proc/sys/kernel/panic_on_oops` is set ([why](faults-and-reclamation.md#fault)), if `fs.suid_dumpable` is not 0 ([why](endovisor.md#abi)), if `debug.exception-trace` is on ([why](virtualizing-ostd/user-mode.md#exceptions)), if the running kernel is a real-time one (`PREEMPT_RT`, which the endovisor reports at load), or if the endovisor's device is missing. Warn, but do not refuse, if the bundle's `cpuset` overlaps a processor on which the operator has placed real-time or otherwise latency-sensitive work, since a sandbox may hold any processor it runs on for up to 2 ms at a time ([why](virtualizing-ostd/scheduling.md#cooperative)); the runtime cannot know all such work, so this is the operator's to arrange.
