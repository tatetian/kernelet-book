...
1. **Check the host.** Refuse if `/proc/sys/kernel/panic_on_oops` is set ([why](faults-and-reclamation.md#fault)), if `fs.suid_dumpable` is not 0 ([why](endovisor.md#abi)), or if the endovisor's device is missing.
