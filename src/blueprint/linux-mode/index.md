...
*A second host for the same kernelets. The Design chapter assumes the host kernel is Asterinas; this chapter asks whether Linux can host kernelets instead, answers yes, and says exactly what it costs. Everything here was either measured on a Linux 6.12 built and booted for the purpose, or cited to Linux's own source with a link. The chapter also changes one decision in the Design chapter, because the mechanism that makes Linux mode possible is better in both modes.*
...
Linux can host kernelets. Three things had to be true, and all three were tested:
...
3. **Everything else maps onto ordinary Linux.** Kernelet tasks are kernel threads, tenant memory is a virtual memory area whose fault handler is the kernelet's, grains are pages from Linux's allocator addressed through its direct map, and a virtual interrupt is a wakeup. None of this needs a patch.
