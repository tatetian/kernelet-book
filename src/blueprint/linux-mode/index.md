...
And one thing a container cannot give: **the tenant's kernel schedules the tenant's threads**. Scheduling has two levels ([Scheduling](virtualizing-ostd/scheduling.md)). Linux decides how much processor time the sandbox gets and when, by its control group's weight and limits; the kernelet's own scheduler, whatever policy the kernel proper implements, decides exactly which of its tasks use that time. Linux never sees more runnable tasks of a sandbox than a number fixed at its creation, its virtual CPUs and a few helpers, so nothing a kernelet's scheduler decides can cost a neighbor anything, and the two cooperate so that Linux does not take a virtual CPU away in the middle of a critical section, within a bound of two milliseconds.
