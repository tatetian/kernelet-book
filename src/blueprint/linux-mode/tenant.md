...
The selector byte lives in the tenant's memory, so the tenant can store *allow* and make an ordinary call. It can jump to the `syscall` instruction inside the stub, which is in the exempt range by construction, with registers of its own choosing. And it can simply **fork or exec**: Linux clears the dispatch setting in `copy_process()` and again in `begin_new_exec()`, and [the manual](https://docs.kernel.org/admin-guide/syscall-user-dispatch.html) says so — any fork or exec of the process resets the mechanism to off. The clearing sits in `dup_task_struct`, which every task creation passes through, so it takes new threads as well as forked processes. Without a patch, only the tenant's *first thread* is ever intercepted. Every child runs with its calls going straight to Linux from its first instruction, and nothing inside the sandbox can re-arm the setting before that instruction runs, because re-arming is itself a system call the kernelet would have to service.

That is disqualifying rather than merely weak, and for two reasons worth separating. A kernelet **is** its tenant's kernel: the tenant's process table, descriptors, mounts and memory live in the kernelet's own state, so a child whose calls go to Linux asks a different kernel for its process id and opens files in the host's file system, and the sandbox stops being one system. And that child is running against Linux's whole system-call surface with its own credentials, so what confines it is the container's namespaces and filter rather than its kernelet. Neither is a corner case: a sandbox is a process tree by construction, an init that forks a shell and a shell that forks every command, so the escape happens at the first `fork`.

The asymmetry is the argument for the patch. Dispatch is state Linux knows about, so Linux resets it. A per-task hook is a field Linux does not know about, so the same routine copies it, and a child is attached before it exists.
...
- **Only one entry point is hooked, and there are four.** A kernel built with 32-bit compatibility — which production kernels are — also enters through the legacy software interrupt, through the 32-bit fast-call instruction, and through the older 32-bit entry instruction. A tenant that enters by an unhooked path has its call serviced by Linux, with its own credentials, invisibly to the kernelet. That is a complete escape, and the guest used here had compatibility compiled out, so it could not appear in the measurement.

  Enumerating them is the wrong fix, and the [alternatives chapter](../alternatives/index.md) gives the right one: every x86 entry funnels through [`syscall_trace_enter()`](https://elixir.bootlin.com/linux/v6.12/source/kernel/entry/common.c#L28), which is where dispatch, ptrace and seccomp already live and where the marker this patch tests is set. One hook there covers every entry by construction, and covers three other architectures with it.
- **A fifth path bypasses the hook even with compatibility off**, though not as completely as this chapter first said. The legacy virtual system-call page at a fixed high address is emulated inside the page-fault handler, which calls three system calls directly and never reaches the dispatch path or the hook. It does, however, call [`secure_computing()`](https://elixir.bootlin.com/linux/v6.12/source/arch/x86/entry/vsyscall/vsyscall_64.c#L216) first, and seccomp's filter input carries the instruction pointer, which on that path is the page's own address. So a short seccomp filter closes it per sandbox, with no patch and no boot setting, and the operator requirement this chapter used to state is withdrawn.
...
**Process lifecycle.** The hook's shape — take a register set, return a value — cannot express `fork`, which returns twice; `execve`, which replaces the address space and the register file; or the return from a signal handler, which restores a saved frame. Only Linux can create a Linux task. But it does not follow, as this page first concluded, that a module cannot own a process: Linux has exported the machinery for that since binary-format handlers became loadable modules, and [Alternative designs](../alternatives/index.md) builds on it. How a kernelet's processes map onto Linux tasks, and who owns the tenant's signals, is **not designed here**, and it was this chapter's largest open item until the next one answered it.
