...
*Five things the Design chapter takes for granted do not hold when the host is Linux: an invariant, an assumption, a property the boundary owes, a mechanism, and one constraint that comes from the hardware rather than from Linux at all. They are collected here rather than scattered, because together they are the honest measure of what Linux mode costs. All five have an answer — three sketched and not built, one measured, and one that arrived from the [alternatives chapter](../alternatives/containment.md#trampoline) after this page was written. Each applies to a kernelet's own kernel threads as much as to a tenant's task, which is why they are not on the [previous page](tenant.md).*
...
## A fault in kernelet code is a Linux oops, unless the host catches it
...
The repair has three parts and none is designed here. The first is not the obvious one: the host's ordinary preemption disable compiles to a compiler barrier on a kernel built without preemption, which is what the chapter's own experiments ran on, so it is a no-op on exactly the configuration a density-minded operator is most likely to run. What does work on every preemption model is disabling the host's software-interrupt processing, and the [seat](../alternatives/execution.md#seat) avoids the question entirely by taking race freedom from possession rather than from any of the host's build-dependent guards. The tenant's task must be held on one processor for the duration of a serviced call, by the primitive that forbids migration rather than by an affinity mask, which is a different thing with a scheduler cost of its own. And the virtual-CPU assignment must be injective at every instant — at most one task inside a serviced call per virtual CPU — which is a scheduling constraint, not a lock.
...
- **Fault containment holds at service-call depth zero, and only there**: a notifier and a landing pad recover a fault in a kernelet's text for one exported symbol ([alternatives](../alternatives/containment.md#trampoline)). What remains is the oops Linux prints before the notifier runs.
