...
| `FrameAllocOptions`, `grains_request` | the host's frame allocator, bounded slice per kernelet | Linux's page allocator up to 4 MiB, the contiguous allocator above that; either may sleep, and either can fail under fragmentation | **to the tenant**: a grant can fail where it would have succeeded |
