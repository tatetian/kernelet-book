...
`UserContext` carries the FS base, which user programs use for thread-local storage. Linux caches it per task and restores it on every context switch, so writing the register alone would be undone. The endovisor writes both the task's saved field and the register, with preemption off between the two, before leaving for user mode. Since the context is per tenant thread and is applied on every departure, the thread pointer follows a thread from one virtual CPU to another with no further work.

Floating-point and vector registers belong to a tenant thread too, and nothing in a register file carries them. To Linux they are the *carrier's* user state: Linux saves them when it switches the carrier out and restores them before the carrier next reaches user mode, and kernelet code is compiled without them, so a round trip through the kernelet leaves them untouched. What Linux cannot know is that the kernelet has put a different thread on the carrier. OSTD already has the place where that is handled: a task that has user state owns an `FpuContext`, which the kernel proper saves when the task is switched out and loads when it is switched in, as well as when it builds or restores a signal frame. vOSTD implements those two operations with the services `fpu_save(area, len)` and `fpu_load(area, len)`. The endovisor brackets each with Linux's [`fpregs_lock()`](https://elixir.bootlin.com/linux/v6.12/source/arch/x86/include/asm/fpu/api.h#L69); if Linux's flag says the registers in the processor are not the carrier's at this moment, it first makes them so with [`switch_fpu_return()`](https://elixir.bootlin.com/linux/v6.12/source/arch/x86/kernel/fpu/core.c#L782), which is exported; then it saves the user-visible components to the kernelet's buffer, or loads them from it, with the processor's own instructions. `fpu_load` checks the buffer's header as Linux checks a signal frame's, so that a kernelet cannot load supervisor state or reserved bits. **[unverified]**: read from the source, not yet built.
...
- **Per switch between tenant threads on one virtual CPU**: two service calls for the floating-point state, and one page-table load if the threads belong to different processes. Linux pays the same two when it switches between its own tasks. The measured figures are on the [Scheduling](scheduling.md) page.
