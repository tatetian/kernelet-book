...
**The first entry.** A new carrier starts in an in-kernel function, not at a system call ([Tasks](tasks.md#carriers)). When its kernelet task first calls `user_run`, that function returns, and Linux's fork-return path would zero the `rax` it is about to restore. The endovisor therefore marks the carrier as having pending work, so that the resume hook runs last and applies the full register file.
...
System calls, signals, demand paging and exceptions behave as they do when Asterinas is the host. Two differences are observable. A program that executes a 32-bit system-call instruction gets `SIGSEGV`, as on Asterinas, where under Linux proper it would have got a system call. And the time a physical interrupt takes while a carrier is in user mode is charged to the sandbox, as it is to any Linux process.
