...
**The periodic tick.** The kernel proper also expects a regular tick, to account time and to run its periodic callbacks. While any carrier of the kernelet holds a seat, the virtual CPU's timer fires at the kernel proper's tick rate and counts the tick in the virtual CPU's shared record; the count is consumed the next time kernelet code passes a check point. When no carrier holds the seat and no deadline is armed, the timer is not armed at all. An idle kernelet costs Linux no timer interrupts, which matters when a machine holds thousands of them.
