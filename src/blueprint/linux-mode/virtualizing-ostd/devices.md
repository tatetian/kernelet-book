...
**The register half** runs inside the service call, on the kernelet's own carrier, charged to the sandbox. It implements the virtio MMIO register file as state in endovisor memory. It never sleeps, because a driver may hold a spin lock across a register write, as it would on hardware. The one register write that does real work is *notify*: the model walks the driver's ring, and for each request checks that every buffer it names lies in the kernelet's grant, using the [owner array](memory.md). It reaches the ring and the buffers through Linux's direct map, because they are frames, not tenant addresses. It copies the request's *description* into a bounded inbox and returns. It moves no data. The walk is bounded by the ring's size, which the model, not the driver, limits: a driver's write to the queue-size register is refused above 256.
...
A reference to a kernelet's frames held by a device thread outlives the service call that created it, so it is counted. [Destroy](../faults-and-reclamation.md#destroy) refuses to free a grant while a count is non-zero, and is retried when the I/O completes or is cancelled.
