...
- **In kernelet code, with virtual interrupts on.** The endovisor stores the interrupted instruction pointer in the record's `upcall_ip`, sets `irq_off`, and points the interrupted frame at the image's **upcall stub**, by the same means as [eviction](../faults-and-reclamation.md#eviction). The stub, which is vOSTD's, realigns the stack (an interrupt lands between any two instructions), pushes `upcall_ip`, every register and the flags onto the *current task's* kernelet stack, and runs the handlers for what is pending. Then it does what a machine's interrupt return should do for a kernel that preempts: it clears `irq_off`, looks at `pending` once more, and, if the scheduler now wants another task and `guards` is zero, switches; with a guard held it returns, and the switch happens when the guard drops. Because the interrupted state is on the task's own stack, and the slot in the record was emptied onto it before anything else could be delivered, the preempted task can resume later on any virtual CPU, by popping what the stub pushed.
...
- **In kernelet code, deep in a stack.** The handlers will run on the interrupted task's kernelet stack, so the endovisor redirects only if the interrupted stack pointer is at least 32 KiB above the task's stack limit: the [16 KiB that belong to Linux](../faults-and-reclamation.md#stack) and 16 KiB (*chosen*) for the handlers. Otherwise the bit waits, as if virtual interrupts were off. A task that is that deep is about to be [stopped for it](../faults-and-reclamation.md#stack) or about to return, and a tick that lands there is not a reason to kill it.
...
`vcpu_kick(i)` is the inter-processor interrupt. OSTD calls it where a machine would send a reschedule interrupt, typically when `enqueue` places a newly runnable task on another CPU's queue. The endovisor sets KICK and then does the least that will get it seen promptly, which depends on where carrier *i* is: asleep in `vcpu_idle`, it is woken; in user mode, it is [flagged](user-mode.md#gate), which interrupts its processor; running kernelet code, its processor is sent the same cross-processor nudge that [eviction](../faults-and-reclamation.md#eviction) uses, which makes the watch timer there fire at once and deliver the upcall; anywhere else, the bit is found on the way back into the kernelet. A kick whose bit is already set does nothing, so a thousand kicks cost one delivery. A higher-priority task that wakes on another virtual CPU therefore preempts within microseconds (*estimated*) unless that virtual CPU is in a critical section, and then when the section ends, as on a machine.

**What OSTD must gain.** Two preemption points, both worth having on a machine too. Today OSTD lets the scheduler preempt a task when it returns from user mode, when it idles, and when it yields (*measured on the tree*: those are the callers of `might_preempt()`); a kernel task that computes is never preempted, and dropping the last guard only decrements the count. The design needs (a) **preemption of kernel-mode code on return from an interrupt**, which is the upcall's last step, and (b) **a check when the outermost guard drops** for a preemption that became due while the guard was held, which is also where Linux is given the processor it [was waiting for](#cooperative). A third, smaller addition is on the [Interrupts and time](interrupts-and-time.md) page: a way for `halt_cpu()` to learn when the next timer expires. Everything else above is a new body for something OSTD already has, and the `Scheduler` and `LocalRunQueue` traits are untouched.
...
The effect is that Linux defers an involuntary preemption of a virtual CPU until the kernelet's critical section ends, by Linux's own mechanism, and that the kernelet gives the processor up the moment it can. On a Linux that does not preempt kernel code at all, the [watch timer](tasks.md#watch) plays Linux's part: it sends the carrier through the yield stub only outside a critical section, and otherwise waits for the section to end. (On a Linux *built* without a preemption count, the word is never read and the mirror is inert; the yield stub is then the whole mechanism.)
...
**The bound: two strikes.** A critical section is a processor that Linux cannot take, so it must not last long, and "must not" has to be enforced against a kernel proper whose logic the tenant may control. The watch timer fires in hard-interrupt context whatever the count. When it finds a carrier in kernelet code, in a critical section, with Linux waiting, it notes the time. If it finds the same at a later tick at least a millisecond after that time, it **forces the yield**: it sends the carrier through the same [yield stub](tasks.md#yield) as on a non-preemptible Linux, which saves every register, switches to the Linux stack, takes the kernelet's increment out of Linux's count as a service stub does, calls `schedule()`, and puts everything back. The grace is therefore at most 2 ms (*chosen*: the watch period, twice), measured by the clock and not by counting ticks that may have landed elsewhere, and it is within what Linux tolerates from its own non-preemptible sections. It is also *borrowed, not stolen*: Linux accounts the extra run time to the sandbox's control group like any other, so the sandbox's long-run share does not grow by a microsecond. What a neighbor can lose is latency, at most the grace, per preemption. Forced yields are counted per sandbox, and a sandbox that collects them is misbehaving in a way the operator can see.
...
**The backstop: do not spin on a lock whose holder is not running.** After a forced yield, or when Linux preempts a virtual CPU in user mode while another spins, a waiter can still meet a lock whose holder is off the processor. vOSTD's spin-lock slow path, after about a thousand failed spins, calls the service `vcpu_on_spin()`, and the endovisor gives the waiter's turn away in favor of a sibling carrier of the same sandbox that is runnable but not running, with Linux's exported [`yield_to()`](https://elixir.bootlin.com/linux/v6.12/source/kernel/sched/syscalls.c#L1468), at most once per tick per virtual CPU and without the option that would interrupt the sibling's processor. This is what KVM does for its guests ([`kvm_vcpu_on_spin()`](https://elixir.bootlin.com/linux/v6.12/source/virt/kvm/kvm_main.c#L4020)), without the hardware that tells KVM a guest is spinning, which vOSTD does not need because it *is* the spin lock. How much it helps is less than the name suggests, and the page says so: in v6.12 the fair class honors a `yield_to` as a hint only when its *next-buddy* feature is on, which by default it is not, so what is certain is only that the waiter gives up its own turn (*measured on the tree*: [`yield_to_task_fair()`](https://elixir.bootlin.com/linux/v6.12/source/kernel/sched/fair.c#L9035)). And it does nothing for a sandbox whose group has spent its `cpu.max` quota: Linux then takes every carrier off the run queues until the period refills, a lock holder among them, and the sandbox stalls for up to a period, 100 ms by default. That stall is the sandbox's own, which is the right party; a container's threads are throttled the same way.
...
3. **The three ways a virtual CPU can resist Linux are each bounded.** A critical section delays preemption by at most 2 ms, charged. Kernelet code that never yields on a non-preemptible Linux is [made to](tasks.md#watch). A kernelet that will not stop at all is [evicted](../faults-and-reclamation.md#eviction).

Three things a kernelet can do reach other processors, and each is bounded. `vcpu_kick` interrupts the target's processor, at most once per pending bit; `vcpu_on_spin` never interrupts anyone, and is limited to one per tick; a `tlb_shootdown` interrupts the processors where the model's address space is loaded, which are the sandbox's own carriers' (Linux tracks that per address space). What remains is the interrupt-time residue the chapter already lists: the watch timer's interrupts, one per millisecond per processor that is running a carrier, are charged to whatever they interrupt, which is the carrier itself, unless the host is built to account interrupt time separately (`IRQ_TIME_ACCOUNTING`), in which case they are charged to nobody, like every other interrupt on such a host.
