...
- **Per Linux preemption that finds a critical section**: at most 2 ms of delay for whoever Linux wanted to run, charged to the sandbox; *measured on the booted prototype* with the earlier form of the bound (a second tick that finds a critical section with Linux waiting), 2,205 µs at the longest against a task that held a guard for 10 ms at a time (two periods and the delivery), and zero involuntary preemptions inside a critical section over 267 million lock acquisitions with the mirror on, against 1,467 over 270 million with it off. The bound as now specified, which reads no record, is **[unverified]** by measurement; it is stricter than the one measured.
