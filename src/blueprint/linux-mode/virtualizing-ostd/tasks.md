...
Linux preempts kernelet code like any other kernel code, if it was built or booted to preempt kernel code at all. Many distribution kernels are not: they reschedule a task in kernel mode only where the code volunteers. Linux's own code volunteers often; a kernelet never does, since it cannot call Linux. Left alone, a tenant that kept its kernel busy could then hold a processor past its control group's limit, and, worse, stall Linux's RCU for the whole machine, because a processor that never reschedules never reports a quiescent state.

So the endovisor volunteers on the kernelet's behalf. The [seat's tick timer](interrupts-and-time.md) fires in hard-interrupt context whatever the preemption model. When it interrupts kernelet text and finds that Linux has marked the carrier as due to reschedule, it points the interrupted frame at a **yield stub**, by the same means as [eviction](../faults-and-reclamation.md#eviction). The stub saves every register on the kernelet stack, switches to the Linux stack, calls Linux's voluntary preemption point (`cond_resched()`, exported), switches back, restores the registers and returns to the interrupted instruction. Kernelet code is compiled without a red zone, as all kernel code is, so nothing below its stack pointer is live. On a Linux that does preempt kernel code the mark is never found set and the stub never runs. **[unverified]** (assumption A29): eviction, which shares the mechanism, is measured; the yield stub, which must also resume, is not built. `disable_preempt()` in vOSTD therefore does not talk to Linux. It raises a per-task **no-preemption counter**, kept in vOSTD, which means three things inside the kernelet and nothing outside it. While it is raised the task keeps its seat, so nobody else touches its per-CPU data. vOSTD will not make a service call that gives up the seat (the service half refuses one with an error, as a check on the kernel proper's own discipline). And it stands in for "interrupts off", which is sound because no kernelet code ever runs in interrupt context. That is all the kernel proper's uses of it need.
