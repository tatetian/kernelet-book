...
The claim that the list is complete is *argued*, not checked: it is complete if every structure that can name a kernelet or a frame of its grant appears in it. On Linux those are the carrier and virtual CPU records, the stack pool, the models with their files and Linux address spaces, the instance's pages and metadata region, the log ring and statistics, the exit status, the device and channel tables, the owner array and the slot table; and step 8 shows that *destroyed* waits for the runtime, since the ring is shared with it.
