...
The claim that the list is complete is *argued*, not checked: it is complete if every structure that can name a kernelet or a frame of its grant appears in it. On Linux those are the carrier and virtual CPU records, the list of kernelet stacks, the models with their files and Linux address spaces, the timers, the device and channel tables, the owner array and the slot table.
...
- **Up to 2 ms of latency per preemption** for whoever else wants a processor a sandbox is on, while a kernelet is in a critical section ([Scheduling](virtualizing-ostd/scheduling.md#cooperative)).
