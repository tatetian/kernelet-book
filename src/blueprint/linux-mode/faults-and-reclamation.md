...
**The root carrier's own life.** The root carrier carries no kernelet task, has no kernelet stack and holds no seat; it lives in its resume hook, cloning carriers on request. It is not counted among the kernelet's carriers and is exempt from the sweep. The [device threads](virtualizing-ostd/devices.md) are threads of the root carrier, so the root carrier is also the one that ends them. After the mark it refuses new requests and waits for the carrier count to reach zero. Then it stops and joins each device thread, with the helper that pairs with the one that created them; a device thread finishes or cancels its Linux I/O before it stops, so that when it is gone it holds no buffer of the kernelet's. Only then does the root carrier record the exit status and end itself. It does not announce *exited*, because it is not gone yet: a task that is ending still has Linux's exit path to run, which closes its descriptors and tears down its address space, calling back into the endovisor as it does. The state moves to *exited*, and whoever is waiting is woken, from the release of the root carrier's [lifeline](virtualizing-ostd/tasks.md#death), which is the last event any task of the sandbox produces. The same goes for the carrier count: a carrier is counted until its lifeline is released, not until it leaves for good. So *exited* has one meaning: no task of the sandbox is left, the root carrier and the device threads included, and nothing of Linux's still refers to the kernelet's memory.
