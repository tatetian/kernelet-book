...
2. **Carriers and stacks.** Assert the carrier count is zero and the root carrier is gone, which is what *exited* means. Then free every kernelet stack the kernelet allocated and did not return; no carrier is left to be standing on one.
...
4. **Timers and virtual interrupts.** Cancel the timers of the kernelet's virtual CPUs and wait for their callbacks to finish. The per-processor [watch timers](virtualizing-ostd/tasks.md#watch) belong to the machine and stop by themselves when they find no carrier to watch.
