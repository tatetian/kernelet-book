...
A kernelet's tasks are the host's tasks, as in Asterinas mode. Here they are **kernel threads**, created on the virtual CPU's processor with `kthread_create_on_cpu()` and woken after they are bound, at a priority the endovisor sets. It must be that call rather than the ordinary one, which wakes the thread first and then refuses to bind it. Binding this way also marks the thread as one whose affinity may not be changed. That does not cost the kernelet a **cpuset**, as this page first said: the refusal is gated on a flag the ordinary affinity call does not pass, so creating the thread unbound and then setting its processors gives one that is both pinned and attachable, which is what Linux's own virtual-machine worker does. The `spawn_task` hook becomes a thread creation; the worker of each virtual CPU is a kernel thread parked on a wait queue.
