...
**The patch as built** for the first three phases of the [prototype](prototype.md#hello), against Linux v6.12: 103 added lines in 8 files and none removed (*measured on the booted prototype*). Of those, 38 are a new header and 12 a configuration option; the code in Linux's paths is the two hunks below, and everything is inert for a task whose pointer is null. With the helper, the export of `mm_alloc` and the one-line selection of preemption notifiers that the second-level scheduler added, the patch is 169 added lines in the same 8 files, none removed (*measured on the booted prototype*, fourth phase).
