...
| `linux.resources.cpu.shares`, `.quota`, `.period` | `cpu_weight` is `shares`, clamped to 2 to 262144, since both are proportional weights; `cpu_quota_us` is `quota` when positive and 0, uncapped, when absent or `-1`; `cpu_period_us` as given |
