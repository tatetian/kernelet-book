...
pub struct InfoPage {
    ...
    pub runs: AtomicU32,
    /// The end of the highest granted run; `max_paddr()` in the kernelet build.
...
pub struct VcpuRecord {
    ...
    /// set if the most recent one found the task in ring 3. The host adds; the kernelet's next
    /// task on this virtual CPU swaps it to zero at a quiescent point and runs the tick
...
- `pt_root_register(root)`, `pt_root_unregister(root)`: the kernelet built a user page table whose root frame is `root`; the host records it so that `pt_activate` and the scheduler accept it, and so that destroy can find every root. Unregistering a root that any task has recorded as its address space, or that is active on any CPU, fails with `-STATE`, which the kernelet build's per-task `Arc` makes impossible in practice ([Memory](virtualizing-ostd/memory.md)); a successful unregister invalidates the root's translations on every CPU it was active on before returning. *Checks:* `root` in the grant and not already registered; the root's 256 kernel-half entries equal the published ones, a backstop against a bug in OSTD (kernelet build). *Cost:* one owner-array read, 256 loads, one insertion in the kernelet's root set; on unregister, a shootdown.
- `pt_activate(root)`: writes CR3 with `root` for the current task and records `root` in the task's host-private state, so that the host's `switch_to_task` restores it on every switch to this task and records the CPU in the root's active set; when the host switches from a kernelet task to a task of another group or to a host task it loads the host kernel's root into CR3, since the tree's `switch_to_task` never writes CR3 and a freed grant frame must never remain a CPU's live root ([Faults, termination, and reclamation](faults-and-reclamation.md)), and clears its own `ACTIVATED_VM_SPACE` cache, so that the host thread's post-schedule handler re-activates its space rather than trusting a stale pointer (checked on the tree: `VmSpace::activate` early-returns when the cache matches). *Checks:* `root` registered, or the kernelet's own kernel page-table root. *Cost:* the CR3 write and, since window mappings are not Global, the loss of the window's translations.
