...
| waking a thread on another CPU and being woken back | 5.8 µs round trip | **about 3 µs per wakeup**, an upper bound: it is a user-space round trip with four system calls |
...
#[repr(C)] pub struct Submit {
    ...
    pub nseg: u16,        // segments lent, at most 16; 0 only for a header-only channel packet
    ...
    pub conn: u32,        // channel devices: the connection the entry belongs to; 0 before one exists
...
A segment is a run of physically contiguous frames inside one grain. Page-cache pages are single frames on the tree (checked: `vm/page_cache/mod.rs`, one `BioSegment` per page), so an entry carries up to sixteen pages, 64 KiB, and `lend-blk` splits larger requests into several entries; the tree's block layer bounds requests at 62 segments (checked: `request_queue.rs`, `max_nr_segments_per_bio`). Every entry has the same size, 536 bytes, which wastes the header field on block entries and keeps the ring arithmetic trivial. **Offload** fields let the NIC compute checksums and split a large segment into packets itself (the tree's virtio-net driver requests the same in its `VirtioNetHdr`).

**Notify.** `dev_notify(dev)` is a service call with no pointer, like every other ([service half](kernelet-api-service.md)). Under the device's lock, taken with interrupts disabled because completions take it too, and without sleeping, the hook copies each new entry out of the submit ring **whole** through `guest_memory` and validates the copy:

- `op` valid for the device; `nseg` at most 16; every `len` nonzero; `id` not in flight (a duplicate is a driver fault that stops the device until reset);
...
Unit costs: an exit that returns to a user-space monitor, 3.7 µs in 2013 and **2–5 µs** today; an exit the kernel part of the hypervisor handles, 1.2 µs in 2013 and **0.5–2 µs** today (both assumption A15, **[unverified]**, scaled from Rizzo's figures); a wakeup about 3 µs measured here, an upper bound; the hook, up to sixteen owner words that may be cold plus one host driver submission, **0.3–2 µs** *estimated*.
...
Left out on purpose: the kernel proper's own hop to its block request thread, inside the tenant's budget on both sides; the reschedule interrupt `raise_irq` sends when a kernelet task occupies the worker's CPU; the host driver's own interrupt handling, which a native process pays too; and the completion writer's own work in that handler, which the first version charged through its device thread and this page leaves uncharged, a gap in invariant I6 to close. The per-byte term is one cold copy (457 ns per 4 KiB) for every microVM row but the virtual function, and none for the kernelet at tier 2: **0.1 core per GB/s**, nothing at a sandbox's usual rate and a whole core at 10 GB/s.
...
- Per device: two rings in the grant, pinned at attach, the inbox, and the host driver's queue entries the device may hold, bounded by the ring size; for tier 1 block, one device thread; for a channel device, the header queues.
...
- Code that does not exist, host side: assumption A14 and steering (A16). Kernelet side: the three drivers, the poll loops, and frame-backed transmit buffers in the kernel proper.
...
- **Submission happens in the notify hook, at most 32 entries per call, non-sleeping, under a lock taken with interrupts disabled, into a `submit_nowait` of the host driver; a device thread exists only for a backend that sleeps** (register D73).
...
- **Network receive is zero-copy only with a per-kernelet receive queue; a shared queue costs one host copy, stated as such; transmit is refused unless the EtherType, the source addresses and the ARP and neighbor-discovery fields are the sandbox's** (register D75).
- **Inter-kernelet transfer copies once, frame to frame, in the sender's hook, into a 16 KiB frame the receiver posted for that connection, credit being those frames; header-only packets take a bounded per-connection queue; moving grains stays the Channels extension** (register D76).
