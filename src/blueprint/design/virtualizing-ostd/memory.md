...
3. Allocates and zeroes eight frames per grain for its metadata, host frames charged to the kernelet, and maps them at `KW_META + paddr / 64`, allocating the level-2 and level-1 tables the range needs for the first time (one level-1 table per 128 MiB of physical address, one level-2 per 64 GiB).
...
Everything the kernelet writes here is in its own grant or in the metadata frames dedicated to it: the metadata slots, the allocator's free lists inside them, and the page-table nodes of its own address spaces, which are grant frames reached through the host's linear map. Everything the host writes is host memory, or the grant at creation through the linear map. That is invariant I2 as this page discharges it, and it needs no exception for page-table nodes and no page-table entry written by the kernelet outside its own user page tables.

The cost is 32 KiB of host memory per grain, 1.56 percent, plus one level-1 frame per 128 MiB of physical address the kernelet's grains touch, for the metadata window alone; the granted frames themselves cost no page-table work at all. A kernelet whose grains are scattered pays a few more page-table frames than one whose grains are adjacent, and the host allocator's preference for adjacent grains keeps the count small. The metadata window reserves address space too, and **how much depends on how a frame's record is found**, which is decision D86. Scaling the raw physical address, as the framework does today, makes the reservation proportional to the physical span the kernelet may be granted — one byte for every sixty-four — and on four-level paging that reservation, not memory, is then what bounds the number of kernelets a machine can hold. Finding the record through a two-level index over a coarse physical section instead, which is how Linux finds its own per-page records when it will not pay for a flat array, makes the region's size a matter of policy rather than of the machine: the bound moves from about two thousand kernelets per TiB of span to about sixty thousand (*estimated*, from the region sizes), and stops growing with the machine. The price is one extra load on `Frame::clone` and `Frame::drop`, which sit on every page fault, and it should be cache-resident. Both functions are internal to the framework, so no public interface moves. The reasoning is in [Alternative designs](../../alternatives/loading.md#metadata); the bound the flat index imposes is worked out in [One address space, many kernelets](../../linux-mode/one-address-space.md). What it buys: the host's metadata array is untouched by kernelets, `Frame::clone` and `drop`, which sit on every page fault and every `mmap`, cost a shift and an add as on the tree, and destroy has nothing per frame to reset.
...
| the metadata window's page-table frames | `meta_pt` | freed: the level-2 and level-1 tables of `KW_META`, and the entries that mapped the image |
...
- **A frame's metadata record is found through a two-level index over a coarse physical section, not by scaling its physical address** (register D86). That is what keeps the region's size a matter of policy rather than of the machine, on both hosts, for one extra load on the two operations that sit on every page fault.
- **Frames are addressed through the host's linear map and metadata through a per-instance window** (register D58, revised; it supersedes D13 and D10, and was itself revised by [Linux as the host](../../linux-mode/one-address-space.md)). The earlier scheme, a physical window at a fixed address inside the kernelet's own top-level entries, is superseded; D58 also supersedes D13 and the kernelet-mapped half of D10. The alternative, a dense slot space with the frame metadata at the head of each run, made `paddr_to_vaddr` and every metadata access a radix lookup on the page-fault path, made the kernelet write page-table entries the host had installed the parents of, and needed a grant table, a radix and reserved level-2 frames that this layout does not. What it gives up is a dense window: the metadata region must span the physical range the kernelet's grains may touch, which reserves address space in proportion to that span and is what bounds the number of kernelets on a machine, and scattered grains cost a few more page-table frames. Assumption A13, which bounded the old physical window instead, is withdrawn; the live bound is A22.
