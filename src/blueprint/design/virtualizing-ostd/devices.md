...
impl VmIoOnce for IoMem {
    fn read_once<T: PodOnce>(&self, offset: usize) -> Result<T> {
        let mut out = 0u64;
        services().mmio_read(self.dev, self.offset + offset as u32, size_of::<T>() as u8, &mut out);
        Ok(T::from_bytes(&out.to_le_bytes()[..size_of::<T>()]))
    }
