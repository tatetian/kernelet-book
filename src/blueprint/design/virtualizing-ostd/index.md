...
| `heap::GlobalHeapAllocator` (hook), `HeapSlot`, `SlotInfo`, `Slab`, `SlabMeta`, `SlabSlotList` | identical | the tree's code over the host's linear map, through `paddr_to_vaddr` | none | nothing |
...
| `VmSpace::new` | virtualized, no crossing | copies the host's 256 kernel-half entries, which `BootArgs` carries, into a root frame from the grant | none beyond today's | nothing |
...
> **To be written.** Review the following earlier-design entries for two-level scheduling in [Synchronization](synchronization.md).
...
| item | kind | how, and what it relies on | cost | what a tenant sees |
|---|---|---|---|---|
| `Task`, `TaskOptions`, `CurrentTask` | virtualized | build creates a Host-managed stack/context resource; run enqueues the internal Task; current reads the vCPU-local Task; yield uses the kernelet scheduler to select a Task for Host context transfer ([Tasks](tasks.md)) | one crossing per resource creation or context transfer; remote wake may notify a vCPU | internal Tasks are selected by the kernelet scheduler |
| `disable_preempt`, `DisabledPreemptGuard` | virtualized, no crossing | prevents internal Task switching on the current vCPU; Host may still preempt its vCPU Thread ([Tasks](tasks.md)) | internal guard operations | nothing |
| `halt_cpu` | virtualized | an internal idle Task checks for work, then calls `vcpu_wait` to block its vCPU Thread ([Tasks](tasks.md#waiting)) | one crossing when waiting is needed | an idle vCPU Thread sleeps |
| `scheduler::inject_scheduler`, `Scheduler`, `LocalRunQueue`, `EnqueueFlags`, `UpdateFlags`, `info::*`, `AtomicCpuId`, `enable_preemption_on_cpu` | virtualized, no crossing | the injected scheduler selects internal Tasks; Host CFS separately schedules vCPU Threads ([Tasks](tasks.md)) | internal scheduling work; context transfers are covered by the Task row | task priority, affinity and scheduler policy govern internal scheduling |
...
| `boot::smp::register_ap_entry` | virtualized | stores the local AP entry for secondary-vCPU startup; each secondary initializes before starting its idle Task ([The rest](the-rest.md#boot)) | startup context per secondary vCPU | nothing |
...
| `#[ostd::main]` | virtualized, no crossing | keeps the native macro expansion; the Task and power backends handle startup and exit ([The rest](the-rest.md#boot)) | none of its own | nothing |
...
1. `init.rs`: retain an idle Task per vCPU; idle waits through `vcpu_wait` ([Tasks](tasks.md#waiting)); the `panic!` that ends the host when init exits becomes `power::exit_with_code(status)` under the feature, so that a kernelet whose init exits ends with `Exited(status)` rather than `Panicked`.
...
5. `thread/kernel_thread.rs`, `syscall/sched_affinity.rs` and the `nice` path retain internal Task attributes instead of forwarding them to Host thread setters; vCPU Thread scheduling attributes are configured separately ([Tasks](tasks.md)).
