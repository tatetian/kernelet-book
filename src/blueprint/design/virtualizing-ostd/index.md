...
| `halt_cpu` | virtualized | `task_park`; unreachable, since the kernelet build spawns no idle threads ([Tasks](tasks.md)) | none | nothing |
...
5. `thread/kernel_thread.rs` (`ThreadOptions::build`), `syscall/sched_affinity.rs` and the `nice` path: one line each forwards a thread's CPU affinity and `nice` to the `TaskOptions` builders and setters the kernelet build adds, so that per-CPU daemons pin correctly and a tenant's `nice` reaches the host ([Tasks](tasks.md)).
