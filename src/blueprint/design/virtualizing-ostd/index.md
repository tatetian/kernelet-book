...
6. `comps/virtio/src/transport/mmio/bus/mod.rs` and `bus/common_device.rs`: the bus allocates the line with `IrqLine::alloc_specific(irq)` and stores an `IrqLine` where the host build maps through the absent `IRQ_CHIP` and stores a `MappedIrqLine` (checked on the tree) ([Devices](devices.md)).
