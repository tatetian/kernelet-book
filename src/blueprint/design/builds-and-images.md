...
Re-encoded, the table holds no address, is shareable, and is identical in every instance, which is how Linux encodes its own. It is a small change to the four stubs and to the walker that reads them; it is worth making on **both** hosts; and on Linux it is the precondition for letting the host's own fixup search find a kernelet's table, which is what gives the kernel proper's fallible copies their contract there ([Alternative designs](../alternatives/index.md)).
...
Which sections hold addresses is not a matter of opinion, so it was measured. A Rust staticlib built with these flags for `x86_64-unknown-none`, containing the shapes a kernel image is full of — tables of trait objects, of string slices and of function pointers — puts **zero** relocations in `.text` (274,307 bytes) and **zero** in `.rodata` (64,914 bytes), and puts all 300 of them in `.data.rel.ro` (181) and `.got` (119); all 300 are of one type, `R_X86_64_RELATIVE`. A C build of the same shapes adds one in `.init_array`. So 98 percent of the read-only material is shareable and the relocated part is a few kilobytes. The measurement is in [Linux-mode experiments](../../notes/linux-mode-experiments.md).
...
10. Every indirect-branch target carries the marker that a processor with indirect-branch tracking requires, so the image runs on such a machine. The design turns on indirect calls through the entry and service tables, so this is not optional. The compiler emits the markers when asked, which was [measured](../linux-mode/evidence.md), and the whole image needs the standard library rebuilt with the same setting. A host whose own build enforces the stricter, type-checked form of the same idea asks more of the image than this check verifies; assumption A19 states what, and it is the one place where a host might reject a kernelet outright. **[unverified]**
