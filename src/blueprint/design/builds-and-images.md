...
2. Every relocation is `R_X86_64_RELATIVE`: *add the instance's base to a value stored in the image*. That is the only type a position-independent image with no imports needs, and it is the only type the measured image contains, so the host's relocation loop has a single case and any other type fails the build.
