...
|---|---|---|---|
| `KW_TEXT` | offset 0 | `.kernelet_entry_table` at offset 4 KiB, then `.text`, `.ex_table`, `.rodata`, `.eh_frame_hdr`, `.eh_frame`, `.gcc_except_table`, `.ktest_array` — **everything that holds no address**, which `.ex_table` does only after the change below | the kind's shared frames, **one physical copy for all instances of the kind**, mapped read-only and executable in each instance's range |
...
*One section has to change to belong here.* On the tree the exception table is an array of pairs of absolute addresses: its entries are `Vaddr` fields (`ostd/src/mm/fault/ex_table.rs`) filled by `.quad` directives in the four fallible-copy stubs (`ostd/src/arch/x86/mm/*_fallible.S`), eight of them in all. Under this decision every one of those is a relocation, so as written the section holds addresses, belongs on the private side by the audit's own rule, and would fail [audit item 8](#audit) — which is the audit doing its job and the region table being wrong.

The fix is to re-encode the table the way Linux encodes its own: each entry a pair of **self-relative** offsets rather than addresses. The table then holds no address, is shareable, and is identical in every instance. It is a small change to the four stubs and to the walker that reads them, it is worth making on **both** hosts, and on Linux it is the precondition for letting the host's own fixup search find a kernelet's table ([Alternative designs](../alternatives/index.md)).
...
9. Every indirect-branch target carries the marker that a processor with indirect-branch tracking requires, so the image runs on such a machine. The design turns on indirect calls through the entry and service tables, so this is not optional. The compiler emits the markers when asked, which was [measured](../linux-mode/evidence.md), and the whole image needs the standard library rebuilt with the same setting. A host whose own build enforces the stricter, type-checked form of the same idea asks more of the image than this check verifies; assumption A19 states what, and it is the one place where a host might reject a kernelet outright. **[unverified]**
