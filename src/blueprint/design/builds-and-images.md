...
8. Every relocation in the image lies inside the per-instance region; `.text` and `.rodata` carry none. Nothing in the image names an address under entry 500, which is reached only through `paddr_to_vaddr`.
